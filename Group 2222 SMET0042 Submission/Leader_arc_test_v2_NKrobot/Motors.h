...
#define L_DIR 16 // This is right
#define R_PWM 9  // This is right 
#define R_DIR 15 // This is right 
...
// Defining High and low for ease of remebering 
#define FWD LOW 
#define REV HIGH
...
class Motors_c {
  ...
  public:
    ...
    void initialise() {
      ...
      // PWM and DIR pins are both sending signals -> OUTPUTS 
      pinMode( L_PWM , OUTPUT );
      pinMode( L_DIR , OUTPUT );
      pinMode( R_PWM , OUTPUT );
      pinMode( R_DIR , OUTPUT );

      // DIR pins HIGH and LOW will be the way the motors spin 
      // -> LOW =
      // -> HIGH = 
      ...
      // Range of 0:255 for the PWM 'power' signal
      // Initialise as 0 to not have motors start moving straight away
      // 255 -> As much as it can (5V)
      // 0 -> Nothing (0V)
      analogWrite( L_PWM , 0 );
      analogWrite( R_PWM , 0 );
    ...
    void setPWM( float left_pwr, float right_pwr ) {
      ...
      // Do these need to be reclassified as int? i.e. does analogue write take only integers?
      ...
      // Checking that max_pwm catch works
      /*
