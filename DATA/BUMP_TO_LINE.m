% % % % Use exactly this style:
% % % %
% % % % x50_y0
% % % % x100_y0
% % % % x100_y20
% % % % x100_y-20
% % % 
% % % clear; clc; close all;
% % % 
% % % file_bump = 'BUMP_STRAIGHT_TESTS.xlsx';             % bump sensor data
% % % file_line = 'LINE_STRAIGHT_TESTS.xlsx';             % line sensor data
% % % 
% % % % Process both datasets
% % % bump = process_dataset(file_bump, 'Bump');
% % % line = process_dataset(file_line, 'Line');
% % % 
% % % % -----------------------------------
% % % % Compare RMS err_x vs x0
% % % % varying lines for y0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare RMS err_x vs x0'); hold on; grid on;
% % % 
% % % all_y0 = unique([bump.y0, line.y0]);
% % % 
% % % for k = 1:length(all_y0)
% % %     y0_sel = all_y0(k);
% % % 
% % %     idxB = find(bump.y0 == y0_sel);
% % %     if ~isempty(idxB)
% % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % %         y_sorted = bump.rms_x(idxB);
% % %         y_sorted = y_sorted(ord);
% % % 
% % %         plot(x_sorted, y_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % %     end
% % % 
% % %     idxL = find(line.y0 == y0_sel);
% % %     if ~isempty(idxL)
% % %         [x_sorted, ord] = sort(line.x0(idxL));
% % %         y_sorted = line.rms_x(idxL);
% % %         y_sorted = y_sorted(ord);
% % % 
% % %         plot(x_sorted, y_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % ylabel('RMS longitudinal error (mm)');
% % % title('Comparison: RMS err_x vs x0');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Compare RMS err_y vs y0
% % % % varying lines for x0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare RMS err_y vs y0'); hold on; grid on;
% % % 
% % % all_x0 = unique([bump.x0, line.x0]);
% % % 
% % % for k = 1:length(all_x0)
% % %     x0_sel = all_x0(k);
% % % 
% % %     idxB = find(bump.x0 == x0_sel);
% % %     if ~isempty(idxB)
% % %         [y_sorted, ord] = sort(bump.y0(idxB));
% % %         rms_sorted = bump.rms_y(idxB);
% % %         rms_sorted = rms_sorted(ord);
% % % 
% % %         plot(y_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% % %     end
% % % 
% % %     idxL = find(line.x0 == x0_sel);
% % %     if ~isempty(idxL)
% % %         [y_sorted, ord] = sort(line.y0(idxL));
% % %         rms_sorted = line.rms_y(idxL);
% % %         rms_sorted = rms_sorted(ord);
% % % 
% % %         plot(y_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded lateral separation y0 (mm)');
% % % ylabel('RMS lateral error (mm)');
% % % title('Comparison: RMS err_y vs y0');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Compare RMS radial error vs x0
% % % % sqrt(err_x^2 + err_y^2)
% % % % varying lines for y0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare RMS radial error vs x0'); hold on; grid on;
% % % 
% % % all_y0 = unique([bump.y0, line.y0]);
% % % 
% % % for k = 1:length(all_y0)
% % %     y0_sel = all_y0(k);
% % % 
% % %     idxB = find(bump.y0 == y0_sel);
% % %     if ~isempty(idxB)
% % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % %         rms_sorted = bump.rms_r(idxB);
% % %         rms_sorted = rms_sorted(ord);
% % % 
% % %         plot(x_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % %     end
% % % 
% % %     idxL = find(line.y0 == y0_sel);
% % %     if ~isempty(idxL)
% % %         [x_sorted, ord] = sort(line.x0(idxL));
% % %         rms_sorted = line.rms_r(idxL);
% % %         rms_sorted = rms_sorted(ord);
% % % 
% % %         plot(x_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % ylabel('RMS radial error (mm)');
% % % title('Comparison: RMS radial error vs x0');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Compare Final err_x vs x0
% % % % varying lines for y0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare Final err_x vs x0'); hold on; grid on;
% % % 
% % % all_y0 = unique([bump.y0, line.y0]);
% % % 
% % % for k = 1:length(all_y0)
% % %     y0_sel = all_y0(k);
% % % 
% % %     idxB = find(bump.y0 == y0_sel);
% % %     if ~isempty(idxB)
% % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % %         final_sorted = bump.final_x(idxB);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % %     end
% % % 
% % %     idxL = find(line.y0 == y0_sel);
% % %     if ~isempty(idxL)
% % %         [x_sorted, ord] = sort(line.x0(idxL));
% % %         final_sorted = line.final_x(idxL);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % ylabel('Final longitudinal error (mm)');
% % % title('Comparison: Final err_x vs x0');
% % % yline(0,'--k');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Compare Final err_y vs y0
% % % % varying lines for x0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare Final err_y vs y0'); hold on; grid on;
% % % 
% % % all_x0 = unique([bump.x0, line.x0]);
% % % 
% % % for k = 1:length(all_x0)
% % %     x0_sel = all_x0(k);
% % % 
% % %     idxB = find(bump.x0 == x0_sel);
% % %     if ~isempty(idxB)
% % %         [y_sorted, ord] = sort(bump.y0(idxB));
% % %         final_sorted = bump.final_y(idxB);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(y_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% % %     end
% % % 
% % %     idxL = find(line.x0 == x0_sel);
% % %     if ~isempty(idxL)
% % %         [y_sorted, ord] = sort(line.y0(idxL));
% % %         final_sorted = line.final_y(idxL);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(y_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded lateral separation y0 (mm)');
% % % ylabel('Final lateral error (mm)');
% % % title('Comparison: Final err_y vs y0');
% % % yline(0,'--k');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Compare Final radial error vs x0
% % % % sqrt(final_err_x^2 + final_err_y^2)
% % % % varying lines for y0
% % % % solid = bump, dashed = line
% % % % -----------------------------------
% % % figure('Name','Compare Final radial error vs x0'); hold on; grid on;
% % % 
% % % all_y0 = unique([bump.y0, line.y0]);
% % % 
% % % for k = 1:length(all_y0)
% % %     y0_sel = all_y0(k);
% % % 
% % %     idxB = find(bump.y0 == y0_sel);
% % %     if ~isempty(idxB)
% % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % %         final_sorted = bump.final_r(idxB);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % %     end
% % % 
% % %     idxL = find(line.y0 == y0_sel);
% % %     if ~isempty(idxL)
% % %         [x_sorted, ord] = sort(line.x0(idxL));
% % %         final_sorted = line.final_r(idxL);
% % %         final_sorted = final_sorted(ord);
% % % 
% % %         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % %     end
% % % end
% % % 
% % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % ylabel('Final radial error (mm)');
% % % title('Comparison: Final radial error vs x0');
% % % legend show;
% % % 
% % % 
% % % % -----------------------------------
% % % % Optional aggregate comparison
% % % % absolute error averaged across all sheets
% % % % -----------------------------------
% % % fprintf('\n=== Aggregate comparison ===\n');
% % % 
% % % fprintf('Bump mean RMS longitudinal error: %.3f mm\n', mean(abs(bump.rms_x)));
% % % fprintf('Line mean RMS longitudinal error: %.3f mm\n', mean(abs(line.rms_x)));
% % % 
% % % fprintf('Bump mean RMS lateral error: %.3f mm\n', mean(abs(bump.rms_y)));
% % % fprintf('Line mean RMS lateral error: %.3f mm\n', mean(abs(line.rms_y)));
% % % 
% % % fprintf('Bump mean RMS radial error: %.3f mm\n', mean(abs(bump.rms_r)));
% % % fprintf('Line mean RMS radial error: %.3f mm\n', mean(abs(line.rms_r)));
% % % 
% % % fprintf('Bump mean |final longitudinal error|: %.3f mm\n', mean(abs(bump.final_x)));
% % % fprintf('Line mean |final longitudinal error|: %.3f mm\n', mean(abs(line.final_x)));
% % % 
% % % fprintf('Bump mean |final lateral error|: %.3f mm\n', mean(abs(bump.final_y)));
% % % fprintf('Line mean |final lateral error|: %.3f mm\n', mean(abs(line.final_y)));
% % % 
% % % fprintf('Bump mean final radial error: %.3f mm\n', mean(abs(bump.final_r)));
% % % fprintf('Line mean final radial error: %.3f mm\n', mean(abs(line.final_r)));
% % % 
% % % if mean(abs(line.rms_y)) < mean(abs(bump.rms_y))
% % %     fprintf('Line sensors appear better laterally in RMS.\n');
% % % else
% % %     fprintf('Bump sensors appear better laterally in RMS.\n');
% % % end
% % % 
% % % if mean(abs(bump.rms_x)) < mean(abs(line.rms_x))
% % %     fprintf('Bump sensors appear better longitudinally in RMS.\n');
% % % else
% % %     fprintf('Line sensors appear better longitudinally in RMS.\n');
% % % end
% % % 
% % % if mean(abs(bump.rms_r)) < mean(abs(line.rms_r))
% % %     fprintf('Bump sensors appear better in radial RMS.\n');
% % % else
% % %     fprintf('Line sensors appear better in radial RMS.\n');
% % % end
% % % 
% % % if mean(abs(line.final_y)) < mean(abs(bump.final_y))
% % %     fprintf('Line sensors appear better laterally in final error.\n');
% % % else
% % %     fprintf('Bump sensors appear better laterally in final error.\n');
% % % end
% % % 
% % % if mean(abs(bump.final_x)) < mean(abs(line.final_x))
% % %     fprintf('Bump sensors appear better longitudinally in final error.\n');
% % % else
% % %     fprintf('Line sensors appear better longitudinally in final error.\n');
% % % end
% % % 
% % % if mean(abs(bump.final_r)) < mean(abs(line.final_r))
% % %     fprintf('Bump sensors appear better in final radial error.\n');
% % % else
% % %     fprintf('Line sensors appear better in final radial error.\n');
% % % end
% % % 
% % % 
% % % % ===================================
% % % % Local function
% % % % ===================================
% % % function out = process_dataset(filename, label)
% % % 
% % %     [~, sheetNames] = xlsfinfo(filename);
% % % 
% % %     out.label = label;
% % %     out.x0 = [];
% % %     out.y0 = [];
% % %     out.rms_x = [];
% % %     out.rms_y = [];
% % %     out.rms_r = [];
% % %     out.final_x = [];
% % %     out.final_y = [];
% % %     out.final_r = [];
% % % 
% % %     figure('Name',['Error X vs Time - ' label]); hold on; grid on;
% % %     xlabel('Time relative to end (ms)');
% % %     ylabel('Longitudinal error (mm)');
% % %     title(['Longitudinal error vs time - ' label]);
% % % 
% % %     figure('Name',['Error Y vs Time - ' label]); hold on; grid on;
% % %     xlabel('Time relative to end (ms)');
% % %     ylabel('Lateral error (mm)');
% % %     title(['Lateral error vs time - ' label]);
% % % 
% % %     figure('Name',['Radial Error vs Time - ' label]); hold on; grid on;
% % %     xlabel('Time relative to end (ms)');
% % %     ylabel('Radial error (mm)');
% % %     title(['Radial error vs time - ' label]);
% % % 
% % %     for s = 1:length(sheetNames)
% % %         sheet = sheetNames{s};
% % % 
% % %         % Parse sheet name like x50_y-20
% % %         tok = regexp(sheet, 'x([-+]?\d+)_y([-+]?\d+)', 'tokens', 'once');
% % %         if isempty(tok)
% % %             fprintf('Skipping sheet "%s" in %s (name not in x##_y## format)\n', sheet, filename);
% % %             continue;
% % %         end
% % % 
% % %         x0 = str2double(tok{1});
% % %         y0 = str2double(tok{2});
% % % 
% % %         data = readmatrix(filename, 'Sheet', sheet);
% % % 
% % %         if size(data,2) < 9
% % %             fprintf('Skipping sheet "%s" in %s (not enough columns)\n', sheet, filename);
% % %             continue;
% % %         end
% % % 
% % %         % Leader B:D
% % %         xL = data(:,2);
% % %         yL = data(:,3);
% % %         tL = data(:,4);
% % % 
% % %         % Follower G:I
% % %         xF = data(:,7);
% % %         yF = data(:,8);
% % %         tF = data(:,9);
% % % 
% % %         % Remove NaNs
% % %         validL = ~isnan(xL) & ~isnan(yL) & ~isnan(tL);
% % %         validF = ~isnan(xF) & ~isnan(yF) & ~isnan(tF);
% % % 
% % %         xL = xL(validL); yL = yL(validL); tL = tL(validL);
% % %         xF = xF(validF); yF = yF(validF); tF = tF(validF);
% % % 
% % %         if numel(xL) < 5 || numel(xF) < 5
% % %             fprintf('Skipping sheet "%s" in %s (too few points)\n', sheet, filename);
% % %             continue;
% % %         end
% % % 
% % %         % -----------------------------------
% % %         % Align by end of motion
% % %         % -----------------------------------
% % %         dxL = [0; diff(xL)];
% % %         dxF = [0; diff(xF)];
% % % 
% % %         moveThresh = 0.5;  % mm per sample
% % %         idxL_end = find(abs(dxL) > moveThresh, 1, 'last');
% % %         idxF_end = find(abs(dxF) > moveThresh, 1, 'last');
% % % 
% % %         if isempty(idxL_end), idxL_end = length(tL); end
% % %         if isempty(idxF_end), idxF_end = length(tF); end
% % % 
% % %         tL_rel = tL - tL(idxL_end);
% % %         tF_rel = tF - tF(idxF_end);
% % % 
% % %         t_start = max(tL_rel(1), tF_rel(1));
% % %         t_end   = min(tL_rel(end), tF_rel(end));
% % % 
% % %         if t_start >= t_end
% % %             fprintf('Skipping sheet "%s" in %s (no overlap after alignment)\n', sheet, filename);
% % %             continue;
% % %         end
% % % 
% % %         t_common = t_start:50:t_end;
% % % 
% % %         xL_i = interp1(tL_rel, xL, t_common, 'linear');
% % %         yL_i = interp1(tL_rel, yL, t_common, 'linear');
% % % 
% % %         xF_i = interp1(tF_rel, xF, t_common, 'linear');
% % %         yF_i = interp1(tF_rel, yF, t_common, 'linear');
% % % 
% % %         % -----------------------------------
% % %         % Put leader into follower coordinate frame
% % %         % Leader reverses, so flip sign first
% % %         % -----------------------------------
% % %         xL_F = -xL_i + x0;
% % %         yL_F = -yL_i + y0;
% % % 
% % %         % -----------------------------------
% % %         % Actual separation
% % %         % -----------------------------------
% % %         sep_x = xL_F - xF_i;
% % %         sep_y = yL_F - yF_i;
% % % 
% % %         % -----------------------------------
% % %         % Error = deviation from commanded/initial separation
% % %         % -----------------------------------
% % %         err_x = sep_x - x0;
% % %         err_y = sep_y - y0;
% % %         err_r = sqrt(err_x.^2 + err_y.^2);
% % % 
% % %         % RMS error
% % %         rms_x = sqrt(mean(err_x.^2));
% % %         rms_y = sqrt(mean(err_y.^2));
% % %         rms_r = sqrt(mean(err_x.^2 + err_y.^2));
% % % 
% % %         % Final error
% % %         final_err_x = err_x(end);
% % %         final_err_y = err_y(end);
% % %         final_err_r = sqrt(final_err_x.^2 + final_err_y.^2);
% % % 
% % %         % Store results
% % %         out.x0(end+1) = x0;
% % %         out.y0(end+1) = y0;
% % %         out.rms_x(end+1) = rms_x;
% % %         out.rms_y(end+1) = rms_y;
% % %         out.rms_r(end+1) = rms_r;
% % %         out.final_x(end+1) = final_err_x;
% % %         out.final_y(end+1) = final_err_y;
% % %         out.final_r(end+1) = final_err_r;
% % % 
% % %         % Time-domain plots
% % %         figure(findobj('Name',['Error X vs Time - ' label]));
% % %         plot(t_common, err_x, 'DisplayName', sheet);
% % % 
% % %         figure(findobj('Name',['Error Y vs Time - ' label]));
% % %         plot(t_common, err_y, 'DisplayName', sheet);
% % % 
% % %         figure(findobj('Name',['Radial Error vs Time - ' label]));
% % %         plot(t_common, err_r, 'DisplayName', sheet);
% % %     end
% % % 
% % %     figure(findobj('Name',['Error X vs Time - ' label]));
% % %     yline(0,'--k');
% % %     legend show;
% % % 
% % %     figure(findobj('Name',['Error Y vs Time - ' label]));
% % %     yline(0,'--k');
% % %     legend show;
% % % 
% % %     figure(findobj('Name',['Radial Error vs Time - ' label]));
% % %     legend show;
% % % end
% % % 
% % % % % Use exactly this style:
% % % % %
% % % % % x50_y0
% % % % % x100_y0
% % % % % x100_y20
% % % % % x100_y-20
% % % % 
% % % % clear; clc; close all;
% % % % 
% % % % file_bump = 'BUMP_STRAIGHT_TESTS.xlsx';        % bump sensor data
% % % % file_line = 'XY_OFFSET_line.xlsx';    % line sensor data
% % % % 
% % % % % Process both datasets
% % % % bump = process_dataset(file_bump, 'Bump');
% % % % line = process_dataset(file_line, 'Line');
% % % % 
% % % % % -----------------------------------
% % % % % Compare RMS err_x vs x0
% % % % % varying lines for y0
% % % % % solid = bump, dashed = line
% % % % % -----------------------------------
% % % % figure('Name','Compare RMS err_x vs x0'); hold on; grid on;
% % % % 
% % % % all_y0 = unique([bump.y0, line.y0]);
% % % % 
% % % % for k = 1:length(all_y0)
% % % %     y0_sel = all_y0(k);
% % % % 
% % % %     idxB = find(bump.y0 == y0_sel);
% % % %     if ~isempty(idxB)
% % % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % % %         y_sorted = bump.rms_x(idxB);
% % % %         y_sorted = y_sorted(ord);
% % % % 
% % % %         plot(x_sorted, y_sorted, '-o', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % % %     end
% % % % 
% % % %     idxL = find(line.y0 == y0_sel);
% % % %     if ~isempty(idxL)
% % % %         [x_sorted, ord] = sort(line.x0(idxL));
% % % %         y_sorted = line.rms_x(idxL);
% % % %         y_sorted = y_sorted(ord);
% % % % 
% % % %         plot(x_sorted, y_sorted, '--s', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % % %     end
% % % % end
% % % % 
% % % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % % ylabel('RMS longitudinal error (mm)');
% % % % title('Comparison: RMS err_x vs x0');
% % % % legend show;
% % % % 
% % % % 
% % % % % -----------------------------------
% % % % % Compare RMS err_y vs y0
% % % % % varying lines for x0
% % % % % solid = bump, dashed = line
% % % % % -----------------------------------
% % % % figure('Name','Compare RMS err_y vs y0'); hold on; grid on;
% % % % 
% % % % all_x0 = unique([bump.x0, line.x0]);
% % % % 
% % % % for k = 1:length(all_x0)
% % % %     x0_sel = all_x0(k);
% % % % 
% % % %     idxB = find(bump.x0 == x0_sel);
% % % %     if ~isempty(idxB)
% % % %         [y_sorted, ord] = sort(bump.y0(idxB));
% % % %         rms_sorted = bump.rms_y(idxB);
% % % %         rms_sorted = rms_sorted(ord);
% % % % 
% % % %         plot(y_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% % % %     end
% % % % 
% % % %     idxL = find(line.x0 == x0_sel);
% % % %     if ~isempty(idxL)
% % % %         [y_sorted, ord] = sort(line.y0(idxL));
% % % %         rms_sorted = line.rms_y(idxL);
% % % %         rms_sorted = rms_sorted(ord);
% % % % 
% % % %         plot(y_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% % % %     end
% % % % end
% % % % 
% % % % xlabel('Commanded lateral separation y0 (mm)');
% % % % ylabel('RMS lateral error (mm)');
% % % % title('Comparison: RMS err_y vs y0');
% % % % legend show;
% % % % 
% % % % 
% % % % % -----------------------------------
% % % % % Compare Final err_x vs x0
% % % % % varying lines for y0
% % % % % solid = bump, dashed = line
% % % % % -----------------------------------
% % % % figure('Name','Compare Final err_x vs x0'); hold on; grid on;
% % % % 
% % % % all_y0 = unique([bump.y0, line.y0]);
% % % % 
% % % % for k = 1:length(all_y0)
% % % %     y0_sel = all_y0(k);
% % % % 
% % % %     idxB = find(bump.y0 == y0_sel);
% % % %     if ~isempty(idxB)
% % % %         [x_sorted, ord] = sort(bump.x0(idxB));
% % % %         final_sorted = bump.final_x(idxB);
% % % %         final_sorted = final_sorted(ord);
% % % % 
% % % %         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% % % %     end
% % % % 
% % % %     idxL = find(line.y0 == y0_sel);
% % % %     if ~isempty(idxL)
% % % %         [x_sorted, ord] = sort(line.x0(idxL));
% % % %         final_sorted = line.final_x(idxL);
% % % %         final_sorted = final_sorted(ord);
% % % % 
% % % %         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% % % %     end
% % % % end
% % % % 
% % % % xlabel('Commanded longitudinal separation x0 (mm)');
% % % % ylabel('Final longitudinal error (mm)');
% % % % title('Comparison: Final err_x vs x0');
% % % % yline(0,'--k');
% % % % legend show;
% % % % 
% % % % 
% % % % % -----------------------------------
% % % % % Compare Final err_y vs y0
% % % % % varying lines for x0
% % % % % solid = bump, dashed = line
% % % % % -----------------------------------
% % % % figure('Name','Compare Final err_y vs y0'); hold on; grid on;
% % % % 
% % % % all_x0 = unique([bump.x0, line.x0]);
% % % % 
% % % % for k = 1:length(all_x0)
% % % %     x0_sel = all_x0(k);
% % % % 
% % % %     idxB = find(bump.x0 == x0_sel);
% % % %     if ~isempty(idxB)
% % % %         [y_sorted, ord] = sort(bump.y0(idxB));
% % % %         final_sorted = bump.final_y(idxB);
% % % %         final_sorted = final_sorted(ord);
% % % % 
% % % %         plot(y_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% % % %     end
% % % % 
% % % %     idxL = find(line.x0 == x0_sel);
% % % %     if ~isempty(idxL)
% % % %         [y_sorted, ord] = sort(line.y0(idxL));
% % % %         final_sorted = line.final_y(idxL);
% % % %         final_sorted = final_sorted(ord);
% % % % 
% % % %         plot(y_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% % % %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% % % %     end
% % % % end
% % % % 
% % % % xlabel('Commanded lateral separation y0 (mm)');
% % % % ylabel('Final lateral error (mm)');
% % % % title('Comparison: Final err_y vs y0');
% % % % yline(0,'--k');
% % % % legend show;
% % % % 
% % % % 
% % % % % -----------------------------------
% % % % % Optional aggregate comparison
% % % % % absolute error averaged across all sheets
% % % % % -----------------------------------
% % % % fprintf('\n=== Aggregate comparison ===\n');
% % % % 
% % % % fprintf('Bump mean RMS longitudinal error: %.3f mm\n', mean(abs(bump.rms_x)));
% % % % fprintf('Line mean RMS longitudinal error: %.3f mm\n', mean(abs(line.rms_x)));
% % % % 
% % % % fprintf('Bump mean RMS lateral error: %.3f mm\n', mean(abs(bump.rms_y)));
% % % % fprintf('Line mean RMS lateral error: %.3f mm\n', mean(abs(line.rms_y)));
% % % % 
% % % % fprintf('Bump mean |final longitudinal error|: %.3f mm\n', mean(abs(bump.final_x)));
% % % % fprintf('Line mean |final longitudinal error|: %.3f mm\n', mean(abs(line.final_x)));
% % % % 
% % % % fprintf('Bump mean |final lateral error|: %.3f mm\n', mean(abs(bump.final_y)));
% % % % fprintf('Line mean |final lateral error|: %.3f mm\n', mean(abs(line.final_y)));
% % % % 
% % % % if mean(abs(line.rms_y)) < mean(abs(bump.rms_y))
% % % %     fprintf('Line sensors appear better laterally in RMS.\n');
% % % % else
% % % %     fprintf('Bump sensors appear better laterally in RMS.\n');
% % % % end
% % % % 
% % % % if mean(abs(bump.rms_x)) < mean(abs(line.rms_x))
% % % %     fprintf('Bump sensors appear better longitudinally in RMS.\n');
% % % % else
% % % %     fprintf('Line sensors appear better longitudinally in RMS.\n');
% % % % end
% % % % 
% % % % if mean(abs(line.final_y)) < mean(abs(bump.final_y))
% % % %     fprintf('Line sensors appear better laterally in final error.\n');
% % % % else
% % % %     fprintf('Bump sensors appear better laterally in final error.\n');
% % % % end
% % % % 
% % % % if mean(abs(bump.final_x)) < mean(abs(line.final_x))
% % % %     fprintf('Bump sensors appear better longitudinally in final error.\n');
% % % % else
% % % %     fprintf('Line sensors appear better longitudinally in final error.\n');
% % % % end
% % % % 
% % % % 
% % % % % ===================================
% % % % % Local function
% % % % % ===================================
% % % % function out = process_dataset(filename, label)
% % % % 
% % % %     [~, sheetNames] = xlsfinfo(filename);
% % % % 
% % % %     out.label = label;
% % % %     out.x0 = [];
% % % %     out.y0 = [];
% % % %     out.rms_x = [];
% % % %     out.rms_y = [];
% % % %     out.final_x = [];
% % % %     out.final_y = [];
% % % % 
% % % %     figure('Name',['Error X vs Time - ' label]); hold on; grid on;
% % % %     xlabel('Time relative to end (ms)');
% % % %     ylabel('Longitudinal error (mm)');
% % % %     title(['Longitudinal error vs time - ' label]);
% % % % 
% % % %     figure('Name',['Error Y vs Time - ' label]); hold on; grid on;
% % % %     xlabel('Time relative to end (ms)');
% % % %     ylabel('Lateral error (mm)');
% % % %     title(['Lateral error vs time - ' label]);
% % % % 
% % % %     for s = 1:length(sheetNames)
% % % %         sheet = sheetNames{s};
% % % % 
% % % %         % Parse sheet name like x50_y-20
% % % %         tok = regexp(sheet, 'x([-+]?\d+)_y([-+]?\d+)', 'tokens', 'once');
% % % %         if isempty(tok)
% % % %             fprintf('Skipping sheet "%s" in %s (name not in x##_y## format)\n', sheet, filename);
% % % %             continue;
% % % %         end
% % % % 
% % % %         x0 = str2double(tok{1});
% % % %         y0 = str2double(tok{2});
% % % % 
% % % %         data = readmatrix(filename, 'Sheet', sheet);
% % % % 
% % % %         if size(data,2) < 9
% % % %             fprintf('Skipping sheet "%s" in %s (not enough columns)\n', sheet, filename);
% % % %             continue;
% % % %         end
% % % % 
% % % %         % Leader B:D
% % % %         xL = data(:,2);
% % % %         yL = data(:,3);
% % % %         tL = data(:,4);
% % % % 
% % % %         % Follower G:I
% % % %         xF = data(:,7);
% % % %         yF = data(:,8);
% % % %         tF = data(:,9);
% % % % 
% % % %         % Remove NaNs
% % % %         validL = ~isnan(xL) & ~isnan(yL) & ~isnan(tL);
% % % %         validF = ~isnan(xF) & ~isnan(yF) & ~isnan(tF);
% % % % 
% % % %         xL = xL(validL); yL = yL(validL); tL = tL(validL);
% % % %         xF = xF(validF); yF = yF(validF); tF = tF(validF);
% % % % 
% % % %         if numel(xL) < 5 || numel(xF) < 5
% % % %             fprintf('Skipping sheet "%s" in %s (too few points)\n', sheet, filename);
% % % %             continue;
% % % %         end
% % % % 
% % % %         % -----------------------------------
% % % %         % Align by end of motion
% % % %         % -----------------------------------
% % % %         dxL = [0; diff(xL)];
% % % %         dxF = [0; diff(xF)];
% % % % 
% % % %         moveThresh = 0.5;  % mm per sample
% % % %         idxL_end = find(abs(dxL) > moveThresh, 1, 'last');
% % % %         idxF_end = find(abs(dxF) > moveThresh, 1, 'last');
% % % % 
% % % %         if isempty(idxL_end), idxL_end = length(tL); end
% % % %         if isempty(idxF_end), idxF_end = length(tF); end
% % % % 
% % % %         tL_rel = tL - tL(idxL_end);
% % % %         tF_rel = tF - tF(idxF_end);
% % % % 
% % % %         t_start = max(tL_rel(1), tF_rel(1));
% % % %         t_end   = min(tL_rel(end), tF_rel(end));
% % % % 
% % % %         if t_start >= t_end
% % % %             fprintf('Skipping sheet "%s" in %s (no overlap after alignment)\n', sheet, filename);
% % % %             continue;
% % % %         end
% % % % 
% % % %         t_common = t_start:50:t_end;
% % % % 
% % % %         xL_i = interp1(tL_rel, xL, t_common, 'linear');
% % % %         yL_i = interp1(tL_rel, yL, t_common, 'linear');
% % % % 
% % % %         xF_i = interp1(tF_rel, xF, t_common, 'linear');
% % % %         yF_i = interp1(tF_rel, yF, t_common, 'linear');
% % % % 
% % % %         % -----------------------------------
% % % %         % Put leader into follower coordinate frame
% % % %         % Leader reverses, so flip sign first
% % % %         % -----------------------------------
% % % %         xL_F = -xL_i + x0;
% % % %         yL_F = -yL_i + y0;
% % % % 
% % % %         % -----------------------------------
% % % %         % Actual separation
% % % %         % -----------------------------------
% % % %         sep_x = xL_F - xF_i;
% % % %         sep_y = yL_F - yF_i;
% % % % 
% % % %         % -----------------------------------
% % % %         % Error = deviation from commanded/initial separation
% % % %         % -----------------------------------
% % % %         err_x = sep_x - x0;
% % % %         err_y = sep_y - y0;
% % % % 
% % % %         % RMS error
% % % %         rms_x = sqrt(mean(err_x.^2));
% % % %         rms_y = sqrt(mean(err_y.^2));
% % % % 
% % % %         % Final error
% % % %         final_err_x = err_x(end);
% % % %         final_err_y = err_y(end);
% % % % 
% % % %         % Store results
% % % %         out.x0(end+1) = x0;
% % % %         out.y0(end+1) = y0;
% % % %         out.rms_x(end+1) = rms_x;
% % % %         out.rms_y(end+1) = rms_y;
% % % %         out.final_x(end+1) = final_err_x;
% % % %         out.final_y(end+1) = final_err_y;
% % % % 
% % % %         % Time-domain plots
% % % %         figure(findobj('Name',['Error X vs Time - ' label]));
% % % %         plot(t_common, err_x, 'DisplayName', sheet);
% % % % 
% % % %         figure(findobj('Name',['Error Y vs Time - ' label]));
% % % %         plot(t_common, err_y, 'DisplayName', sheet);
% % % %     end
% % % % 
% % % %     figure(findobj('Name',['Error X vs Time - ' label]));
% % % %     yline(0,'--k');
% % % %     legend show;
% % % % 
% % % %     figure(findobj('Name',['Error Y vs Time - ' label]));
% % % %     yline(0,'--k');
% % % %     legend show;
% % % % end
% % 
% % % Use exactly this style:
% % %
% % % x50_y0
% % % x100_y0
% % % x100_y20
% % % x100_y-20
% % 
% % clear; clc; close all;
% % 
% % file_bump = 'BUMP_STRAIGHT_TESTS.xlsx';             % bump sensor data
% % file_line = 'LINE_STRAIGHT_TESTS.xlsx';             % line sensor data
% % 
% % % Process both datasets
% % bump = process_dataset(file_bump, 'Bump');
% % line = process_dataset(file_line, 'Line');
% % 
% % % -----------------------------------
% % % Compare RMS err_x vs x0
% % % varying lines for y0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare RMS err_x vs x0'); hold on; grid on;
% % 
% % all_y0 = unique([bump.y0, line.y0]);
% % 
% % for k = 1:length(all_y0)
% %     y0_sel = all_y0(k);
% % 
% %     idxB = find(bump.y0 == y0_sel);
% %     if ~isempty(idxB)
% %         [x_sorted, ord] = sort(bump.x0(idxB));
% %         y_sorted = bump.rms_x(idxB);
% %         y_sorted = y_sorted(ord);
% % 
% %         plot(x_sorted, y_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% %     end
% % 
% %     idxL = find(line.y0 == y0_sel);
% %     if ~isempty(idxL)
% %         [x_sorted, ord] = sort(line.x0(idxL));
% %         y_sorted = line.rms_x(idxL);
% %         y_sorted = y_sorted(ord);
% % 
% %         plot(x_sorted, y_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded longitudinal separation x0 (mm)');
% % ylabel('RMS longitudinal error (mm)');
% % title('Comparison: RMS err_x vs x0');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Compare RMS err_y vs y0
% % % varying lines for x0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare RMS err_y vs y0'); hold on; grid on;
% % 
% % all_x0 = unique([bump.x0, line.x0]);
% % 
% % for k = 1:length(all_x0)
% %     x0_sel = all_x0(k);
% % 
% %     idxB = find(bump.x0 == x0_sel);
% %     if ~isempty(idxB)
% %         [y_sorted, ord] = sort(bump.y0(idxB));
% %         rms_sorted = bump.rms_y(idxB);
% %         rms_sorted = rms_sorted(ord);
% % 
% %         plot(y_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% %     end
% % 
% %     idxL = find(line.x0 == x0_sel);
% %     if ~isempty(idxL)
% %         [y_sorted, ord] = sort(line.y0(idxL));
% %         rms_sorted = line.rms_y(idxL);
% %         rms_sorted = rms_sorted(ord);
% % 
% %         plot(y_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded lateral separation y0 (mm)');
% % ylabel('RMS lateral error (mm)');
% % title('Comparison: RMS err_y vs y0');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Compare RMS radial error vs x0
% % % sqrt(err_x^2 + err_y^2)
% % % varying lines for y0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare RMS radial error vs x0'); hold on; grid on;
% % 
% % all_y0 = unique([bump.y0, line.y0]);
% % 
% % for k = 1:length(all_y0)
% %     y0_sel = all_y0(k);
% % 
% %     idxB = find(bump.y0 == y0_sel);
% %     if ~isempty(idxB)
% %         [x_sorted, ord] = sort(bump.x0(idxB));
% %         rms_sorted = bump.rms_r(idxB);
% %         rms_sorted = rms_sorted(ord);
% % 
% %         plot(x_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% %     end
% % 
% %     idxL = find(line.y0 == y0_sel);
% %     if ~isempty(idxL)
% %         [x_sorted, ord] = sort(line.x0(idxL));
% %         rms_sorted = line.rms_r(idxL);
% %         rms_sorted = rms_sorted(ord);
% % 
% %         plot(x_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded longitudinal separation x0 (mm)');
% % ylabel('RMS radial error (mm)');
% % title('Comparison: RMS radial error vs x0');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Single plot:
% % % RMS radial error for Line sensors
% % % RMS radial error for Bump sensors
% % % with shaded envelopes = +-1 s.d.
% % % aggregated by x0
% % % -----------------------------------
% % figure('Name','Mean RMS radial error vs x0 with envelopes'); hold on; grid on;
% % 
% % all_x0 = unique([bump.x0, line.x0]);
% % 
% % bump_mean = nan(size(all_x0));
% % bump_std  = nan(size(all_x0));
% % line_mean = nan(size(all_x0));
% % line_std  = nan(size(all_x0));
% % 
% % for k = 1:length(all_x0)
% %     x0_sel = all_x0(k);
% % 
% %     idxB = bump.x0 == x0_sel;
% %     if any(idxB)
% %         bump_mean(k) = mean(bump.rms_r(idxB));
% %         bump_std(k)  = std(bump.rms_r(idxB));
% %     end
% % 
% %     idxL = line.x0 == x0_sel;
% %     if any(idxL)
% %         line_mean(k) = mean(line.rms_r(idxL));
% %         line_std(k)  = std(line.rms_r(idxL));
% %     end
% % end
% % 
% % validB = ~isnan(bump_mean);
% % xB = all_x0(validB);
% % mB = bump_mean(validB);
% % sB = bump_std(validB);
% % 
% % validL = ~isnan(line_mean);
% % xL = all_x0(validL);
% % mL = line_mean(validL);
% % sL = line_std(validL);
% % 
% % % Draw shaded envelopes first
% % fill([xB fliplr(xB)], [mB+sB fliplr(mB-sB)], ...
% %     [0 0.4470 0.7410], ...
% %     'FaceAlpha', 0.18, 'EdgeColor', 'none', ...
% %     'HandleVisibility', 'off');
% % 
% % fill([xL fliplr(xL)], [mL+sL fliplr(mL-sL)], ...
% %     [0.8500 0.3250 0.0980], ...
% %     'FaceAlpha', 0.18, 'EdgeColor', 'none', ...
% %     'HandleVisibility', 'off');
% % 
% % plot(xB, mB, '-o', 'LineWidth', 1.8, ...
% %     'DisplayName', 'Bump RMS radial error');
% % 
% % plot(xL, mL, '--s', 'LineWidth', 1.8, ...
% %     'DisplayName', 'Line RMS radial error');
% % 
% % xlabel('Commanded longitudinal separation x0 (mm)');
% % ylabel('RMS radial error (mm)');
% % title('RMS radial error vs x0 with \pm1 s.d. envelopes');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Compare Final err_x vs x0
% % % varying lines for y0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare Final err_x vs x0'); hold on; grid on;
% % 
% % all_y0 = unique([bump.y0, line.y0]);
% % 
% % for k = 1:length(all_y0)
% %     y0_sel = all_y0(k);
% % 
% %     idxB = find(bump.y0 == y0_sel);
% %     if ~isempty(idxB)
% %         [x_sorted, ord] = sort(bump.x0(idxB));
% %         final_sorted = bump.final_x(idxB);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% %     end
% % 
% %     idxL = find(line.y0 == y0_sel);
% %     if ~isempty(idxL)
% %         [x_sorted, ord] = sort(line.x0(idxL));
% %         final_sorted = line.final_x(idxL);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded longitudinal separation x0 (mm)');
% % ylabel('Final longitudinal error (mm)');
% % title('Comparison: Final err_x vs x0');
% % yline(0,'--k');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Compare Final err_y vs y0
% % % varying lines for x0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare Final err_y vs y0'); hold on; grid on;
% % 
% % all_x0 = unique([bump.x0, line.x0]);
% % 
% % for k = 1:length(all_x0)
% %     x0_sel = all_x0(k);
% % 
% %     idxB = find(bump.x0 == x0_sel);
% %     if ~isempty(idxB)
% %         [y_sorted, ord] = sort(bump.y0(idxB));
% %         final_sorted = bump.final_y(idxB);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(y_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
% %     end
% % 
% %     idxL = find(line.x0 == x0_sel);
% %     if ~isempty(idxL)
% %         [y_sorted, ord] = sort(line.y0(idxL));
% %         final_sorted = line.final_y(idxL);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(y_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded lateral separation y0 (mm)');
% % ylabel('Final lateral error (mm)');
% % title('Comparison: Final err_y vs y0');
% % yline(0,'--k');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Compare Final radial error vs x0
% % % sqrt(final_err_x^2 + final_err_y^2)
% % % varying lines for y0
% % % solid = bump, dashed = line
% % % -----------------------------------
% % figure('Name','Compare Final radial error vs x0'); hold on; grid on;
% % 
% % all_y0 = unique([bump.y0, line.y0]);
% % 
% % for k = 1:length(all_y0)
% %     y0_sel = all_y0(k);
% % 
% %     idxB = find(bump.y0 == y0_sel);
% %     if ~isempty(idxB)
% %         [x_sorted, ord] = sort(bump.x0(idxB));
% %         final_sorted = bump.final_r(idxB);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
% %     end
% % 
% %     idxL = find(line.y0 == y0_sel);
% %     if ~isempty(idxL)
% %         [x_sorted, ord] = sort(line.x0(idxL));
% %         final_sorted = line.final_r(idxL);
% %         final_sorted = final_sorted(ord);
% % 
% %         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
% %             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
% %     end
% % end
% % 
% % xlabel('Commanded longitudinal separation x0 (mm)');
% % ylabel('Final radial error (mm)');
% % title('Comparison: Final radial error vs x0');
% % legend show;
% % 
% % 
% % % -----------------------------------
% % % Optional aggregate comparison
% % % absolute error averaged across all sheets
% % % -----------------------------------
% % fprintf('\n=== Aggregate comparison ===\n');
% % 
% % fprintf('Bump mean RMS longitudinal error: %.3f mm\n', mean(abs(bump.rms_x)));
% % fprintf('Line mean RMS longitudinal error: %.3f mm\n', mean(abs(line.rms_x)));
% % 
% % fprintf('Bump mean RMS lateral error: %.3f mm\n', mean(abs(bump.rms_y)));
% % fprintf('Line mean RMS lateral error: %.3f mm\n', mean(abs(line.rms_y)));
% % 
% % fprintf('Bump mean RMS radial error: %.3f mm\n', mean(abs(bump.rms_r)));
% % fprintf('Line mean RMS radial error: %.3f mm\n', mean(abs(line.rms_r)));
% % 
% % fprintf('Bump mean |final longitudinal error|: %.3f mm\n', mean(abs(bump.final_x)));
% % fprintf('Line mean |final longitudinal error|: %.3f mm\n', mean(abs(line.final_x)));
% % 
% % fprintf('Bump mean |final lateral error|: %.3f mm\n', mean(abs(bump.final_y)));
% % fprintf('Line mean |final lateral error|: %.3f mm\n', mean(abs(line.final_y)));
% % 
% % fprintf('Bump mean final radial error: %.3f mm\n', mean(abs(bump.final_r)));
% % fprintf('Line mean final radial error: %.3f mm\n', mean(abs(line.final_r)));
% % 
% % if mean(abs(line.rms_y)) < mean(abs(bump.rms_y))
% %     fprintf('Line sensors appear better laterally in RMS.\n');
% % else
% %     fprintf('Bump sensors appear better laterally in RMS.\n');
% % end
% % 
% % if mean(abs(bump.rms_x)) < mean(abs(line.rms_x))
% %     fprintf('Bump sensors appear better longitudinally in RMS.\n');
% % else
% %     fprintf('Line sensors appear better longitudinally in RMS.\n');
% % end
% % 
% % if mean(abs(bump.rms_r)) < mean(abs(line.rms_r))
% %     fprintf('Bump sensors appear better in radial RMS.\n');
% % else
% %     fprintf('Line sensors appear better in radial RMS.\n');
% % end
% % 
% % if mean(abs(line.final_y)) < mean(abs(bump.final_y))
% %     fprintf('Line sensors appear better laterally in final error.\n');
% % else
% %     fprintf('Bump sensors appear better laterally in final error.\n');
% % end
% % 
% % if mean(abs(bump.final_x)) < mean(abs(line.final_x))
% %     fprintf('Bump sensors appear better longitudinally in final error.\n');
% % else
% %     fprintf('Line sensors appear better longitudinally in final error.\n');
% % end
% % 
% % if mean(abs(bump.final_r)) < mean(abs(line.final_r))
% %     fprintf('Bump sensors appear better in final radial error.\n');
% % else
% %     fprintf('Line sensors appear better in final radial error.\n');
% % end
% % 
% % 
% % % ===================================
% % % Local function
% % % ===================================
% % function out = process_dataset(filename, label)
% % 
% %     [~, sheetNames] = xlsfinfo(filename);
% % 
% %     out.label = label;
% %     out.x0 = [];
% %     out.y0 = [];
% %     out.rms_x = [];
% %     out.rms_y = [];
% %     out.rms_r = [];
% %     out.final_x = [];
% %     out.final_y = [];
% %     out.final_r = [];
% % 
% %     figure('Name',['Error X vs Time - ' label]); hold on; grid on;
% %     xlabel('Time relative to end (ms)');
% %     ylabel('Longitudinal error (mm)');
% %     title(['Longitudinal error vs time - ' label]);
% % 
% %     figure('Name',['Error Y vs Time - ' label]); hold on; grid on;
% %     xlabel('Time relative to end (ms)');
% %     ylabel('Lateral error (mm)');
% %     title(['Lateral error vs time - ' label]);
% % 
% %     figure('Name',['Radial Error vs Time - ' label]); hold on; grid on;
% %     xlabel('Time relative to end (ms)');
% %     ylabel('Radial error (mm)');
% %     title(['Radial error vs time - ' label]);
% % 
% %     for s = 1:length(sheetNames)
% %         sheet = sheetNames{s};
% % 
% %         % Parse sheet name like x50_y-20
% %         tok = regexp(sheet, 'x([-+]?\d+)_y([-+]?\d+)', 'tokens', 'once');
% %         if isempty(tok)
% %             fprintf('Skipping sheet "%s" in %s (name not in x##_y## format)\n', sheet, filename);
% %             continue;
% %         end
% % 
% %         x0 = str2double(tok{1});
% %         y0 = str2double(tok{2});
% % 
% %         data = readmatrix(filename, 'Sheet', sheet);
% % 
% %         if size(data,2) < 9
% %             fprintf('Skipping sheet "%s" in %s (not enough columns)\n', sheet, filename);
% %             continue;
% %         end
% % 
% %         % Leader B:D
% %         xL = data(:,2);
% %         yL = data(:,3);
% %         tL = data(:,4);
% % 
% %         % Follower G:I
% %         xF = data(:,7);
% %         yF = data(:,8);
% %         tF = data(:,9);
% % 
% %         % Remove NaNs
% %         validL = ~isnan(xL) & ~isnan(yL) & ~isnan(tL);
% %         validF = ~isnan(xF) & ~isnan(yF) & ~isnan(tF);
% % 
% %         xL = xL(validL); yL = yL(validL); tL = tL(validL);
% %         xF = xF(validF); yF = yF(validF); tF = tF(validF);
% % 
% %         if numel(xL) < 5 || numel(xF) < 5
% %             fprintf('Skipping sheet "%s" in %s (too few points)\n', sheet, filename);
% %             continue;
% %         end
% % 
% %         % -----------------------------------
% %         % Align by end of motion
% %         % -----------------------------------
% %         dxL = [0; diff(xL)];
% %         dxF = [0; diff(xF)];
% % 
% %         moveThresh = 0.5;  % mm per sample
% %         idxL_end = find(abs(dxL) > moveThresh, 1, 'last');
% %         idxF_end = find(abs(dxF) > moveThresh, 1, 'last');
% % 
% %         if isempty(idxL_end), idxL_end = length(tL); end
% %         if isempty(idxF_end), idxF_end = length(tF); end
% % 
% %         tL_rel = tL - tL(idxL_end);
% %         tF_rel = tF - tF(idxF_end);
% % 
% %         t_start = max(tL_rel(1), tF_rel(1));
% %         t_end   = min(tL_rel(end), tF_rel(end));
% % 
% %         if t_start >= t_end
% %             fprintf('Skipping sheet "%s" in %s (no overlap after alignment)\n', sheet, filename);
% %             continue;
% %         end
% % 
% %         t_common = t_start:50:t_end;
% % 
% %         xL_i = interp1(tL_rel, xL, t_common, 'linear');
% %         yL_i = interp1(tL_rel, yL, t_common, 'linear');
% % 
% %         xF_i = interp1(tF_rel, xF, t_common, 'linear');
% %         yF_i = interp1(tF_rel, yF, t_common, 'linear');
% % 
% %         % -----------------------------------
% %         % Put leader into follower coordinate frame
% %         % Leader reverses, so flip sign first
% %         % -----------------------------------
% %         xL_F = -xL_i + x0;
% %         yL_F = -yL_i + y0;
% % 
% %         % -----------------------------------
% %         % Actual separation
% %         % -----------------------------------
% %         sep_x = xL_F - xF_i;
% %         sep_y = yL_F - yF_i;
% % 
% %         % -----------------------------------
% %         % Error = deviation from commanded/initial separation
% %         % -----------------------------------
% %         err_x = sep_x - x0;
% %         err_y = sep_y - y0;
% %         err_r = sqrt(err_x.^2 + err_y.^2);
% % 
% %         % RMS error
% %         rms_x = sqrt(mean(err_x.^2));
% %         rms_y = sqrt(mean(err_y.^2));
% %         rms_r = sqrt(mean(err_x.^2 + err_y.^2));
% % 
% %         % Final error
% %         final_err_x = err_x(end);
% %         final_err_y = err_y(end);
% %         final_err_r = sqrt(final_err_x.^2 + final_err_y.^2);
% % 
% %         % Store results
% %         out.x0(end+1) = x0;
% %         out.y0(end+1) = y0;
% %         out.rms_x(end+1) = rms_x;
% %         out.rms_y(end+1) = rms_y;
% %         out.rms_r(end+1) = rms_r;
% %         out.final_x(end+1) = final_err_x;
% %         out.final_y(end+1) = final_err_y;
% %         out.final_r(end+1) = final_err_r;
% % 
% %         % Time-domain plots
% %         figure(findobj('Name',['Error X vs Time - ' label]));
% %         plot(t_common, err_x, 'DisplayName', sheet);
% % 
% %         figure(findobj('Name',['Error Y vs Time - ' label]));
% %         plot(t_common, err_y, 'DisplayName', sheet);
% % 
% %         figure(findobj('Name',['Radial Error vs Time - ' label]));
% %         plot(t_common, err_r, 'DisplayName', sheet);
% %     end
% % 
% %     figure(findobj('Name',['Error X vs Time - ' label]));
% %     yline(0,'--k');
% %     legend show;
% % 
% %     figure(findobj('Name',['Error Y vs Time - ' label]));
% %     yline(0,'--k');
% %     legend show;
% % 
% %     figure(findobj('Name',['Radial Error vs Time - ' label]));
% %     legend show;
% % end
% 
% % Use exactly this style:
% %
% % x100_y-20
% % x105_y-10
% % x110_y0
% % x115_y10
% % x120_y20
% 
% clear; clc; close all;
% 
% file_bump = 'BUMP_STRAIGHT_TESTS.xlsx';             % bump sensor data
% file_line = 'LINE_STRAIGHT_TESTS.xlsx';             % line sensor data
% 
% % Process both datasets
% bump = process_dataset(file_bump, 'Bump');
% line = process_dataset(file_line, 'Line');
% 
% % -----------------------------------
% % Compare RMS err_x vs x0
% % varying lines for y0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare RMS err_x vs x0'); hold on; grid on;
% 
% all_y0 = unique([bump.y0, line.y0]);
% 
% for k = 1:length(all_y0)
%     y0_sel = all_y0(k);
% 
%     idxB = find(bump.y0 == y0_sel);
%     if ~isempty(idxB)
%         [x_sorted, ord] = sort(bump.x0(idxB));
%         y_sorted = bump.rms_x(idxB);
%         y_sorted = y_sorted(ord);
% 
%         plot(x_sorted, y_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
%     end
% 
%     idxL = find(line.y0 == y0_sel);
%     if ~isempty(idxL)
%         [x_sorted, ord] = sort(line.x0(idxL));
%         y_sorted = line.rms_x(idxL);
%         y_sorted = y_sorted(ord);
% 
%         plot(x_sorted, y_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS longitudinal error (mm)');
% title('Comparison: RMS err_x vs x0');
% legend show;
% 
% 
% % -----------------------------------
% % Compare RMS err_y vs y0
% % varying lines for x0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare RMS err_y vs y0'); hold on; grid on;
% 
% all_x0 = unique([bump.x0, line.x0]);
% 
% for k = 1:length(all_x0)
%     x0_sel = all_x0(k);
% 
%     idxB = find(bump.x0 == x0_sel);
%     if ~isempty(idxB)
%         [y_sorted, ord] = sort(bump.y0(idxB));
%         rms_sorted = bump.rms_y(idxB);
%         rms_sorted = rms_sorted(ord);
% 
%         plot(y_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
%     end
% 
%     idxL = find(line.x0 == x0_sel);
%     if ~isempty(idxL)
%         [y_sorted, ord] = sort(line.y0(idxL));
%         rms_sorted = line.rms_y(idxL);
%         rms_sorted = rms_sorted(ord);
% 
%         plot(y_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
%     end
% end
% 
% xlabel('Commanded lateral separation y0 (mm)');
% ylabel('RMS lateral error (mm)');
% title('Comparison: RMS err_y vs y0');
% legend show;
% 
% 
% % -----------------------------------
% % Compare RMS radial error vs x0
% % sqrt(err_x^2 + err_y^2)
% % varying lines for y0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare RMS radial error vs x0'); hold on; grid on;
% 
% all_y0 = unique([bump.y0, line.y0]);
% 
% for k = 1:length(all_y0)
%     y0_sel = all_y0(k);
% 
%     idxB = find(bump.y0 == y0_sel);
%     if ~isempty(idxB)
%         [x_sorted, ord] = sort(bump.x0(idxB));
%         rms_sorted = bump.rms_r(idxB);
%         rms_sorted = rms_sorted(ord);
% 
%         plot(x_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
%     end
% 
%     idxL = find(line.y0 == y0_sel);
%     if ~isempty(idxL)
%         [x_sorted, ord] = sort(line.x0(idxL));
%         rms_sorted = line.rms_r(idxL);
%         rms_sorted = rms_sorted(ord);
% 
%         plot(x_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS radial error (mm)');
% title('Comparison: RMS radial error vs x0');
% legend show;
% 
% 
% % -----------------------------------
% % Line sensors only:
% % RMS radial error vs longitudinal separation
% % separate lines for each lateral separation
% % shaded envelopes = +-1 mm
% % -----------------------------------
% x_vals = [100 105 110 115 120];
% y_vals = [-20 -10 0 10 20];
% 
% line_rms_mat = nan(length(y_vals), length(x_vals));
% bump_rms_mat = nan(length(y_vals), length(x_vals));
% 
% for iy = 1:length(y_vals)
%     for ix = 1:length(x_vals)
%         x0_sel = x_vals(ix);
%         y0_sel = y_vals(iy);
% 
%         idxL = (line.x0 == x0_sel) & (line.y0 == y0_sel);
%         if any(idxL)
%             line_rms_mat(iy, ix) = line.rms_r(find(idxL, 1, 'first'));
%         end
% 
%         idxB = (bump.x0 == x0_sel) & (bump.y0 == y0_sel);
%         if any(idxB)
%             bump_rms_mat(iy, ix) = bump.rms_r(find(idxB, 1, 'first'));
%         end
%     end
% end
% 
% % Constant uncertainty = +-1 mm
% line_unc_plus  = ones(size(line_rms_mat));
% line_unc_minus = ones(size(line_rms_mat));
% bump_unc_plus  = ones(size(bump_rms_mat));
% bump_unc_minus = ones(size(bump_rms_mat));
% 
% figure('Name','Line RMS radial error vs longitudinal separation'); 
% hold on; grid on;
% 
% for iy = 1:length(y_vals)
%     y0_sel = y_vals(iy);
% 
%     y_mean  = line_rms_mat(iy, :);
%     y_upper = y_mean + line_unc_plus(iy, :);
%     y_lower = y_mean - line_unc_minus(iy, :);
% 
%     valid = ~isnan(y_mean);
% 
%     if any(valid)
%         x_plot     = x_vals(valid);
%         mean_plot  = y_mean(valid);
%         upper_plot = y_upper(valid);
%         lower_plot = y_lower(valid);
% 
%         fill([x_plot fliplr(x_plot)], ...
%              [upper_plot fliplr(lower_plot)], ...
%              [0.8500 0.3250 0.0980], ...
%              'FaceAlpha', 0.15, ...
%              'EdgeColor', 'none', ...
%              'HandleVisibility', 'off');
% 
%         plot(x_plot, mean_plot, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS radial error (mm)');
% title('Line sensors: RMS radial error vs longitudinal separation (\pm1 mm)');
% legend show;
% 
% 
% % -----------------------------------
% % Bump sensors only:
% % RMS radial error vs longitudinal separation
% % separate lines for each lateral separation
% % shaded envelopes = +-1 mm
% % -----------------------------------
% figure('Name','Bump RMS radial error vs longitudinal separation'); 
% hold on; grid on;
% 
% for iy = 1:length(y_vals)
%     y0_sel = y_vals(iy);
% 
%     y_mean  = bump_rms_mat(iy, :);
%     y_upper = y_mean + bump_unc_plus(iy, :);
%     y_lower = y_mean - bump_unc_minus(iy, :);
% 
%     valid = ~isnan(y_mean);
% 
%     if any(valid)
%         x_plot     = x_vals(valid);
%         mean_plot  = y_mean(valid);
%         upper_plot = y_upper(valid);
%         lower_plot = y_lower(valid);
% 
%         fill([x_plot fliplr(x_plot)], ...
%              [upper_plot fliplr(lower_plot)], ...
%              [0 0.4470 0.7410], ...
%              'FaceAlpha', 0.15, ...
%              'EdgeColor', 'none', ...
%              'HandleVisibility', 'off');
% 
%         plot(x_plot, mean_plot, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS radial error (mm)');
% title('Bump sensors: RMS radial error vs longitudinal separation (\pm1 mm)');
% legend show;
% 
% 
% % -----------------------------------
% % Compare Final err_x vs x0
% % varying lines for y0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare Final err_x vs x0'); hold on; grid on;
% 
% all_y0 = unique([bump.y0, line.y0]);
% 
% for k = 1:length(all_y0)
%     y0_sel = all_y0(k);
% 
%     idxB = find(bump.y0 == y0_sel);
%     if ~isempty(idxB)
%         [x_sorted, ord] = sort(bump.x0(idxB));
%         final_sorted = bump.final_x(idxB);
%         final_sorted = final_sorted(ord);
% 
%         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
%     end
% 
%     idxL = find(line.y0 == y0_sel);
%     if ~isempty(idxL)
%         [x_sorted, ord] = sort(line.x0(idxL));
%         final_sorted = line.final_x(idxL);
%         final_sorted = final_sorted(ord);
% 
%         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('Final longitudinal error (mm)');
% title('Comparison: Final err_x vs x0');
% yline(0,'--k');
% legend show;
% 
% 
% % -----------------------------------
% % Compare Final err_y vs y0
% % varying lines for x0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare Final err_y vs y0'); hold on; grid on;
% 
% all_x0 = unique([bump.x0, line.x0]);
% 
% for k = 1:length(all_x0)
%     x0_sel = all_x0(k);
% 
%     idxB = find(bump.x0 == x0_sel);
%     if ~isempty(idxB)
%         [y_sorted, ord] = sort(bump.y0(idxB));
%         final_sorted = bump.final_y(idxB);
%         final_sorted = final_sorted(ord);
% 
%         plot(y_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
%     end
% 
%     idxL = find(line.x0 == x0_sel);
%     if ~isempty(idxL)
%         [y_sorted, ord] = sort(line.y0(idxL));
%         final_sorted = line.final_y(idxL);
%         final_sorted = final_sorted(ord);
% 
%         plot(y_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
%     end
% end
% 
% xlabel('Commanded lateral separation y0 (mm)');
% ylabel('Final lateral error (mm)');
% title('Comparison: Final err_y vs y0');
% yline(0,'--k');
% legend show;
% 
% 
% % -----------------------------------
% % Compare Final radial error vs x0
% % sqrt(final_err_x^2 + final_err_y^2)
% % varying lines for y0
% % solid = bump, dashed = line
% % -----------------------------------
% figure('Name','Compare Final radial error vs x0'); hold on; grid on;
% 
% all_y0 = unique([bump.y0, line.y0]);
% 
% for k = 1:length(all_y0)
%     y0_sel = all_y0(k);
% 
%     idxB = find(bump.y0 == y0_sel);
%     if ~isempty(idxB)
%         [x_sorted, ord] = sort(bump.x0(idxB));
%         final_sorted = bump.final_r(idxB);
%         final_sorted = final_sorted(ord);
% 
%         plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
%     end
% 
%     idxL = find(line.y0 == y0_sel);
%     if ~isempty(idxL)
%         [x_sorted, ord] = sort(line.x0(idxL));
%         final_sorted = line.final_r(idxL);
%         final_sorted = final_sorted(ord);
% 
%         plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
%             'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
%     end
% end
% 
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('Final radial error (mm)');
% title('Comparison: Final radial error vs x0');
% legend show;
% 
% 
% % -----------------------------------
% % Optional aggregate comparison
% % absolute error averaged across all sheets
% % -----------------------------------
% fprintf('\n=== Aggregate comparison ===\n');
% 
% fprintf('Bump mean RMS longitudinal error: %.3f mm\n', mean(abs(bump.rms_x)));
% fprintf('Line mean RMS longitudinal error: %.3f mm\n', mean(abs(line.rms_x)));
% 
% fprintf('Bump mean RMS lateral error: %.3f mm\n', mean(abs(bump.rms_y)));
% fprintf('Line mean RMS lateral error: %.3f mm\n', mean(abs(line.rms_y)));
% 
% fprintf('Bump mean RMS radial error: %.3f mm\n', mean(abs(bump.rms_r)));
% fprintf('Line mean RMS radial error: %.3f mm\n', mean(abs(line.rms_r)));
% 
% fprintf('Bump mean |final longitudinal error|: %.3f mm\n', mean(abs(bump.final_x)));
% fprintf('Line mean |final longitudinal error|: %.3f mm\n', mean(abs(line.final_x)));
% 
% fprintf('Bump mean |final lateral error|: %.3f mm\n', mean(abs(bump.final_y)));
% fprintf('Line mean |final lateral error|: %.3f mm\n', mean(abs(line.final_y)));
% 
% fprintf('Bump mean final radial error: %.3f mm\n', mean(abs(bump.final_r)));
% fprintf('Line mean final radial error: %.3f mm\n', mean(abs(line.final_r)));
% 
% if mean(abs(line.rms_y)) < mean(abs(bump.rms_y))
%     fprintf('Line sensors appear better laterally in RMS.\n');
% else
%     fprintf('Bump sensors appear better laterally in RMS.\n');
% end
% 
% if mean(abs(bump.rms_x)) < mean(abs(line.rms_x))
%     fprintf('Bump sensors appear better longitudinally in RMS.\n');
% else
%     fprintf('Line sensors appear better longitudinally in RMS.\n');
% end
% 
% if mean(abs(bump.rms_r)) < mean(abs(line.rms_r))
%     fprintf('Bump sensors appear better in radial RMS.\n');
% else
%     fprintf('Line sensors appear better in radial RMS.\n');
% end
% 
% if mean(abs(line.final_y)) < mean(abs(bump.final_y))
%     fprintf('Line sensors appear better laterally in final error.\n');
% else
%     fprintf('Bump sensors appear better laterally in final error.\n');
% end
% 
% if mean(abs(bump.final_x)) < mean(abs(line.final_x))
%     fprintf('Bump sensors appear better longitudinally in final error.\n');
% else
%     fprintf('Line sensors appear better longitudinally in final error.\n');
% end
% 
% if mean(abs(bump.final_r)) < mean(abs(line.final_r))
%     fprintf('Bump sensors appear better in final radial error.\n');
% else
%     fprintf('Line sensors appear better in final radial error.\n');
% end
% 
% 
% % ===================================
% % Local function
% % ===================================
% function out = process_dataset(filename, label)
% 
%     [~, sheetNames] = xlsfinfo(filename);
% 
%     out.label = label;
%     out.x0 = [];
%     out.y0 = [];
%     out.rms_x = [];
%     out.rms_y = [];
%     out.rms_r = [];
%     out.final_x = [];
%     out.final_y = [];
%     out.final_r = [];
% 
%     figure('Name',['Error X vs Time - ' label]); hold on; grid on;
%     xlabel('Time relative to end (ms)');
%     ylabel('Longitudinal error (mm)');
%     title(['Longitudinal error vs time - ' label]);
% 
%     figure('Name',['Error Y vs Time - ' label]); hold on; grid on;
%     xlabel('Time relative to end (ms)');
%     ylabel('Lateral error (mm)');
%     title(['Lateral error vs time - ' label]);
% 
%     figure('Name',['Radial Error vs Time - ' label]); hold on; grid on;
%     xlabel('Time relative to end (ms)');
%     ylabel('Radial error (mm)');
%     title(['Radial error vs time - ' label]);
% 
%     for s = 1:length(sheetNames)
%         sheet = sheetNames{s};
% 
%         % Parse sheet name like x100_y-20
%         tok = regexp(sheet, 'x([-+]?\d+)_y([-+]?\d+)', 'tokens', 'once');
%         if isempty(tok)
%             fprintf('Skipping sheet "%s" in %s (name not in x##_y## format)\n', sheet, filename);
%             continue;
%         end
% 
%         x0 = str2double(tok{1});
%         y0 = str2double(tok{2});
% 
%         data = readmatrix(filename, 'Sheet', sheet);
% 
%         if size(data,2) < 9
%             fprintf('Skipping sheet "%s" in %s (not enough columns)\n', sheet, filename);
%             continue;
%         end
% 
%         % Leader B:D
%         xL = data(:,2);
%         yL = data(:,3);
%         tL = data(:,4);
% 
%         % Follower G:I
%         xF = data(:,7);
%         yF = data(:,8);
%         tF = data(:,9);
% 
%         % Remove NaNs
%         validL = ~isnan(xL) & ~isnan(yL) & ~isnan(tL);
%         validF = ~isnan(xF) & ~isnan(yF) & ~isnan(tF);
% 
%         xL = xL(validL); yL = yL(validL); tL = tL(validL);
%         xF = xF(validF); yF = yF(validF); tF = tF(validF);
% 
%         if numel(xL) < 5 || numel(xF) < 5
%             fprintf('Skipping sheet "%s" in %s (too few points)\n', sheet, filename);
%             continue;
%         end
% 
%         % -----------------------------------
%         % Align by end of motion
%         % -----------------------------------
%         dxL = [0; diff(xL)];
%         dxF = [0; diff(xF)];
% 
%         moveThresh = 0.5;  % mm per sample
%         idxL_end = find(abs(dxL) > moveThresh, 1, 'last');
%         idxF_end = find(abs(dxF) > moveThresh, 1, 'last');
% 
%         if isempty(idxL_end), idxL_end = length(tL); end
%         if isempty(idxF_end), idxF_end = length(tF); end
% 
%         tL_rel = tL - tL(idxL_end);
%         tF_rel = tF - tF(idxF_end);
% 
%         t_start = max(tL_rel(1), tF_rel(1));
%         t_end   = min(tL_rel(end), tF_rel(end));
% 
%         if t_start >= t_end
%             fprintf('Skipping sheet "%s" in %s (no overlap after alignment)\n', sheet, filename);
%             continue;
%         end
% 
%         t_common = t_start:50:t_end;
% 
%         xL_i = interp1(tL_rel, xL, t_common, 'linear');
%         yL_i = interp1(tL_rel, yL, t_common, 'linear');
% 
%         xF_i = interp1(tF_rel, xF, t_common, 'linear');
%         yF_i = interp1(tF_rel, yF, t_common, 'linear');
% 
%         % -----------------------------------
%         % Put leader into follower coordinate frame
%         % Leader reverses, so flip sign first
%         % -----------------------------------
%         xL_F = -xL_i + x0;
%         yL_F = -yL_i + y0;
% 
%         % -----------------------------------
%         % Actual separation
%         % -----------------------------------
%         sep_x = xL_F - xF_i;
%         sep_y = yL_F - yF_i;
% 
%         % -----------------------------------
%         % Error = deviation from commanded/initial separation
%         % -----------------------------------
%         err_x = sep_x - x0;
%         err_y = sep_y - y0;
%         err_r = sqrt(err_x.^2 + err_y.^2);
% 
%         % RMS error
%         rms_x = sqrt(mean(err_x.^2));
%         rms_y = sqrt(mean(err_y.^2));
%         rms_r = sqrt(mean(err_x.^2 + err_y.^2));
% 
%         % Final error
%         final_err_x = err_x(end);
%         final_err_y = err_y(end);
%         final_err_r = sqrt(final_err_x.^2 + final_err_y.^2);
% 
%         % Store results
%         out.x0(end+1) = x0;
%         out.y0(end+1) = y0;
%         out.rms_x(end+1) = rms_x;
%         out.rms_y(end+1) = rms_y;
%         out.rms_r(end+1) = rms_r;
%         out.final_x(end+1) = final_err_x;
%         out.final_y(end+1) = final_err_y;
%         out.final_r(end+1) = final_err_r;
% 
%         % Time-domain plots
%         figure(findobj('Name',['Error X vs Time - ' label]));
%         plot(t_common, err_x, 'DisplayName', sheet);
% 
%         figure(findobj('Name',['Error Y vs Time - ' label]));
%         plot(t_common, err_y, 'DisplayName', sheet);
% 
%         figure(findobj('Name',['Radial Error vs Time - ' label]));
%         plot(t_common, err_r, 'DisplayName', sheet);
%     end
% 
%     figure(findobj('Name',['Error X vs Time - ' label]));
%     yline(0,'--k');
%     legend show;
% 
%     figure(findobj('Name',['Error Y vs Time - ' label]));
%     yline(0,'--k');
%     legend show;
% 
%     figure(findobj('Name',['Radial Error vs Time - ' label]));
%     legend show;
% end

% Use exactly this style:
%
% x100_y-20
% x105_y-10
% x110_y0
% x115_y10
% x120_y20

clear; clc; close all;

file_bump = 'BUMP_STRAIGHT_TESTS.xlsx';             % bump sensor data
file_line = 'LINE_STRAIGHT_TESTS.xlsx';             % line sensor data

% Process both datasets
bump = process_dataset(file_bump, 'Bump');
line = process_dataset(file_line, 'Line');

% -----------------------------------
% Compare RMS err_x vs x0
% varying lines for y0
% solid = bump, dashed = line
% -----------------------------------
figure('Name','Compare RMS err_x vs x0'); hold on; grid on;

all_y0 = unique([bump.y0, line.y0]);

for k = 1:length(all_y0)
    y0_sel = all_y0(k);

    idxB = find(bump.y0 == y0_sel);
    if ~isempty(idxB)
        [x_sorted, ord] = sort(bump.x0(idxB));
        y_sorted = bump.rms_x(idxB);
        y_sorted = y_sorted(ord);

        plot(x_sorted, y_sorted, '-o', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
    end

    idxL = find(line.y0 == y0_sel);
    if ~isempty(idxL)
        [x_sorted, ord] = sort(line.x0(idxL));
        y_sorted = line.rms_x(idxL);
        y_sorted = y_sorted(ord);

        plot(x_sorted, y_sorted, '--s', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
    end
end

xlabel('Commanded longitudinal separation x0 (mm)');
ylabel('RMS longitudinal error (mm)');
title('Comparison: RMS err_x vs x0');
legend show;


% -----------------------------------
% Compare RMS err_y vs y0
% varying lines for x0
% solid = bump, dashed = line
% -----------------------------------
figure('Name','Compare RMS err_y vs y0'); hold on; grid on;

all_x0 = unique([bump.x0, line.x0]);

for k = 1:length(all_x0)
    x0_sel = all_x0(k);

    idxB = find(bump.x0 == x0_sel);
    if ~isempty(idxB)
        [y_sorted, ord] = sort(bump.y0(idxB));
        rms_sorted = bump.rms_y(idxB);
        rms_sorted = rms_sorted(ord);

        plot(y_sorted, rms_sorted, '-o', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
    end

    idxL = find(line.x0 == x0_sel);
    if ~isempty(idxL)
        [y_sorted, ord] = sort(line.y0(idxL));
        rms_sorted = line.rms_y(idxL);
        rms_sorted = rms_sorted(ord);

        plot(y_sorted, rms_sorted, '--s', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
    end
end

xlabel('Commanded lateral separation y0 (mm)');
ylabel('RMS lateral error (mm)');
title('Comparison: RMS err_y vs y0');
legend show;
% -----------------------------------
% Compare RMS radial error vs x0
% varying lines for y0
% solid = bump, dashed = line
% shaded envelopes use supplied uncertainties
% shading matches each line colour
% -----------------------------------
figure('Name','Compare RMS radial error vs x0'); hold on; grid on;

x_vals = [100 105 110 115 120];
y_vals = [-20 -10 0 10 20];

cmp_colors = lines(length(y_vals));

for iy = 1:length(y_vals)
    y0_sel = y_vals(iy);
    this_color = cmp_colors(iy,:);

    % ---- Bump ----
    idxB = (bump.y0 == y0_sel);
    if any(idxB)
        [x_sorted, ord] = sort(bump.x0(idxB));
        rms_sorted = bump.rms_r(idxB);
        rms_sorted = rms_sorted(ord);

        % find matching row in uncertainty matrix
        [~, locB] = ismember(x_sorted, x_vals);

        upperB = rms_sorted + bump_unc_plus(iy, locB);
        lowerB = rms_sorted + bump_unc_minus(iy, locB);   % minus matrix is already negative

        fill([x_sorted fliplr(x_sorted)], ...
             [upperB fliplr(lowerB)], ...
             this_color, ...
             'FaceAlpha', 0.12, ...
             'EdgeColor', 'none', ...
             'HandleVisibility', 'off');

        plot(x_sorted, rms_sorted, '-o', ...
            'Color', this_color, ...
            'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
    end

    % ---- Line ----
    idxL = (line.y0 == y0_sel);
    if any(idxL)
        [x_sorted, ord] = sort(line.x0(idxL));
        rms_sorted = line.rms_r(idxL);
        rms_sorted = rms_sorted(ord);

        [~, locL] = ismember(x_sorted, x_vals);

        upperL = rms_sorted + line_unc_plus(iy, locL);
        lowerL = rms_sorted + line_unc_minus(iy, locL);   % minus matrix is already negative

        fill([x_sorted fliplr(x_sorted)], ...
             [upperL fliplr(lowerL)], ...
             this_color, ...
             'FaceAlpha', 0.12, ...
             'EdgeColor', 'none', ...
             'HandleVisibility', 'off');

        plot(x_sorted, rms_sorted, '--s', ...
            'Color', this_color, ...
            'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
    end
end

xlabel('Initial Longitudinal Separation, X_0 (mm)');
ylabel('RMS radial error (mm)');
legend show;
lgd = legend;
lgd.Title.String = 'Sensor configuration and lateral offset';
% -----------------------------------
% Line sensors only:
% RMS radial error vs longitudinal separation
% separate lines for each lateral separation
% shaded envelopes = +-1 mm
% shading matches line colour
% -----------------------------------
x_vals = [100 105 110 115 120];
y_vals = [-20 -10 0 10 20];

line_rms_mat = nan(length(y_vals), length(x_vals));
bump_rms_mat = nan(length(y_vals), length(x_vals));

for iy = 1:length(y_vals)
    for ix = 1:length(x_vals)
        x0_sel = x_vals(ix);
        y0_sel = y_vals(iy);

        idxL = (line.x0 == x0_sel) & (line.y0 == y0_sel);
        if any(idxL)
            line_rms_mat(iy, ix) = line.rms_r(find(idxL, 1, 'first'));
        end

        idxB = (bump.x0 == x0_sel) & (bump.y0 == y0_sel);
        if any(idxB)
            bump_rms_mat(iy, ix) = bump.rms_r(find(idxB, 1, 'first'));
        end
    end
end

% Constant uncertainty = +-1 mm
line_unc_plus  = [0.84 2.76 3.18 1.07 1.62;
            1.93 3.44 0.77 1.39 2.85;
            1.57 0.91 3.29 2.21 0.68;
            2.98 1.26 0.83 3.47 1.54;
            1.03 2.48 3.07 0.71 1.79];
line_unc_minus = abs([-2.64 -0.82 -2.17 -3.08 -0.96;
            -1.36 -3.49 -0.58 -2.53 -3.11;
            -1.08 -1.87 -3.26 -0.73 -2.41;
            -3.22 -1.19 -0.88 -3.45 -1.68;
            -0.69 -2.74 -3.03 -0.93 -1.95]);
bump_unc_plus  = [2.91 1.14 3.22 2.05 0.73;
            3.34 1.67 0.98 2.76 1.89;
            1.02 3.11 1.58 2.43 0.86;
            2.27 0.59 3.48 1.36 2.64;
            3.05 1.72 0.81 2.93 1.47];
bump_unc_minus = abs([-1.73 -2.88 -0.94 -2.11 -3.27;
            -1.05 -1.92 -2.63 -0.76 -2.44;
            -3.31 -0.62 -1.58 -2.95 -1.14;
            -2.36 -3.12 -0.89 -1.97 -2.68;
            -0.71 -2.53 -3.41 -1.08 -1.76]);
%
figure('Name','Line RMS radial error vs longitudinal separation');
hold on; grid on;

line_colors = lines(length(y_vals));

for iy = 1:length(y_vals)
    y0_sel = y_vals(iy);
    this_color = line_colors(iy,:);

    y_mean  = line_rms_mat(iy, :);
    y_upper = y_mean + line_unc_plus(iy, :);
    y_lower = y_mean - line_unc_minus(iy, :);

    valid = ~isnan(y_mean);

    if any(valid)
        x_plot     = x_vals(valid);
        mean_plot  = y_mean(valid);
        upper_plot = y_upper(valid);
        lower_plot = y_lower(valid);

        fill([x_plot fliplr(x_plot)], ...
             [upper_plot fliplr(lower_plot)], ...
             this_color, ...
             'FaceAlpha', 0.15, ...
             'EdgeColor', 'none', ...
             'HandleVisibility', 'off');

        plot(x_plot, mean_plot, '--s', ...
            'Color', this_color, ...
            'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
    end
end

xlabel('Commanded longitudinal separation x0 (mm)');
ylabel('RMS radial error (mm)');
title('Line sensors: RMS radial error vs longitudinal separation (\pm1 mm)');
legend show;


% -----------------------------------
% Bump sensors only:
% RMS radial error vs longitudinal separation
% separate lines for each lateral separation
% shaded envelopes = +-1 mm
% shading matches line colour
% -----------------------------------
figure('Name','Bump RMS radial error vs longitudinal separation');
hold on; grid on;

bump_colors = lines(length(y_vals));

for iy = 1:length(y_vals)
    y0_sel = y_vals(iy);
    this_color = bump_colors(iy,:);

    y_mean  = bump_rms_mat(iy, :);
    y_upper = y_mean + bump_unc_plus(iy, :);
    y_lower = y_mean - bump_unc_minus(iy, :);

    valid = ~isnan(y_mean);

    if any(valid)
        x_plot     = x_vals(valid);
        mean_plot  = y_mean(valid);
        upper_plot = y_upper(valid);
        lower_plot = y_lower(valid);

        fill([x_plot fliplr(x_plot)], ...
             [upper_plot fliplr(lower_plot)], ...
             this_color, ...
             'FaceAlpha', 0.15, ...
             'EdgeColor', 'none', ...
             'HandleVisibility', 'off');

        plot(x_plot, mean_plot, '-o', ...
            'Color', this_color, ...
            'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
    end
end

xlabel('Commanded longitudinal separation x0 (mm)');
ylabel('RMS radial error (mm)');
title('Bump sensors: RMS radial error vs longitudinal separation (\pm1 mm)');
legend show;


% -----------------------------------
% Compare Final err_x vs x0
% varying lines for y0
% solid = bump, dashed = line
% -----------------------------------
figure('Name','Compare Final err_x vs x0'); hold on; grid on;

all_y0 = unique([bump.y0, line.y0]);

for k = 1:length(all_y0)
    y0_sel = all_y0(k);

    idxB = find(bump.y0 == y0_sel);
    if ~isempty(idxB)
        [x_sorted, ord] = sort(bump.x0(idxB));
        final_sorted = bump.final_x(idxB);
        final_sorted = final_sorted(ord);

        plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
    end

    idxL = find(line.y0 == y0_sel);
    if ~isempty(idxL)
        [x_sorted, ord] = sort(line.x0(idxL));
        final_sorted = line.final_x(idxL);
        final_sorted = final_sorted(ord);

        plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
    end
end

xlabel('Commanded longitudinal separation x0 (mm)');
ylabel('Final longitudinal error (mm)');
title('Comparison: Final err_x vs x0');
yline(0,'--k');
legend show;


% -----------------------------------
% Compare Final err_y vs y0
% varying lines for x0
% solid = bump, dashed = line
% -----------------------------------
figure('Name','Compare Final err_y vs y0'); hold on; grid on;

all_x0 = unique([bump.x0, line.x0]);

for k = 1:length(all_x0)
    x0_sel = all_x0(k);

    idxB = find(bump.x0 == x0_sel);
    if ~isempty(idxB)
        [y_sorted, ord] = sort(bump.y0(idxB));
        final_sorted = bump.final_y(idxB);
        final_sorted = final_sorted(ord);

        plot(y_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, x0 = %g mm', x0_sel));
    end

    idxL = find(line.x0 == x0_sel);
    if ~isempty(idxL)
        [y_sorted, ord] = sort(line.y0(idxL));
        final_sorted = line.final_y(idxL);
        final_sorted = final_sorted(ord);

        plot(y_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, x0 = %g mm', x0_sel));
    end
end

xlabel('Commanded lateral separation y0 (mm)');
ylabel('Final lateral error (mm)');
title('Comparison: Final err_y vs y0');
yline(0,'--k');
legend show;


% -----------------------------------
% Compare Final radial error vs x0
% sqrt(final_err_x^2 + final_err_y^2)
% varying lines for y0
% solid = bump, dashed = line
% -----------------------------------
figure('Name','Compare Final radial error vs x0'); hold on; grid on;

all_y0 = unique([bump.y0, line.y0]);

for k = 1:length(all_y0)
    y0_sel = all_y0(k);

    idxB = find(bump.y0 == y0_sel);
    if ~isempty(idxB)
        [x_sorted, ord] = sort(bump.x0(idxB));
        final_sorted = bump.final_r(idxB);
        final_sorted = final_sorted(ord);

        plot(x_sorted, final_sorted, '-o', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Bump, y0 = %g mm', y0_sel));
    end

    idxL = find(line.y0 == y0_sel);
    if ~isempty(idxL)
        [x_sorted, ord] = sort(line.x0(idxL));
        final_sorted = line.final_r(idxL);
        final_sorted = final_sorted(ord);

        plot(x_sorted, final_sorted, '--s', 'LineWidth', 1.5, ...
            'DisplayName', sprintf('Line, y0 = %g mm', y0_sel));
    end
end

xlabel('Commanded longitudinal separation x0 (mm)');
ylabel('Final radial error (mm)');
title('Comparison: Final radial error vs x0');
legend show;


% -----------------------------------
% Optional aggregate comparison
% absolute error averaged across all sheets
% -----------------------------------
fprintf('\n=== Aggregate comparison ===\n');

fprintf('Bump mean RMS longitudinal error: %.3f mm\n', mean(abs(bump.rms_x)));
fprintf('Line mean RMS longitudinal error: %.3f mm\n', mean(abs(line.rms_x)));

fprintf('Bump mean RMS lateral error: %.3f mm\n', mean(abs(bump.rms_y)));
fprintf('Line mean RMS lateral error: %.3f mm\n', mean(abs(line.rms_y)));

fprintf('Bump mean RMS radial error: %.3f mm\n', mean(abs(bump.rms_r)));
fprintf('Line mean RMS radial error: %.3f mm\n', mean(abs(line.rms_r)));

fprintf('Bump mean |final longitudinal error|: %.3f mm\n', mean(abs(bump.final_x)));
fprintf('Line mean |final longitudinal error|: %.3f mm\n', mean(abs(line.final_x)));

fprintf('Bump mean |final lateral error|: %.3f mm\n', mean(abs(bump.final_y)));
fprintf('Line mean |final lateral error|: %.3f mm\n', mean(abs(line.final_y)));

fprintf('Bump mean final radial error: %.3f mm\n', mean(abs(bump.final_r)));
fprintf('Line mean final radial error: %.3f mm\n', mean(abs(line.final_r)));

if mean(abs(line.rms_y)) < mean(abs(bump.rms_y))
    fprintf('Line sensors appear better laterally in RMS.\n');
else
    fprintf('Bump sensors appear better laterally in RMS.\n');
end

if mean(abs(bump.rms_x)) < mean(abs(line.rms_x))
    fprintf('Bump sensors appear better longitudinally in RMS.\n');
else
    fprintf('Line sensors appear better longitudinally in RMS.\n');
end

if mean(abs(bump.rms_r)) < mean(abs(line.rms_r))
    fprintf('Bump sensors appear better in radial RMS.\n');
else
    fprintf('Line sensors appear better in radial RMS.\n');
end

if mean(abs(line.final_y)) < mean(abs(bump.final_y))
    fprintf('Line sensors appear better laterally in final error.\n');
else
    fprintf('Bump sensors appear better laterally in final error.\n');
end

if mean(abs(bump.final_x)) < mean(abs(line.final_x))
    fprintf('Bump sensors appear better longitudinally in final error.\n');
else
    fprintf('Line sensors appear better longitudinally in final error.\n');
end

if mean(abs(bump.final_r)) < mean(abs(line.final_r))
    fprintf('Bump sensors appear better in final radial error.\n');
else
    fprintf('Line sensors appear better in final radial error.\n');
end


% ===================================
% Local function
% ===================================
function out = process_dataset(filename, label)

    [~, sheetNames] = xlsfinfo(filename);

    out.label = label;
    out.x0 = [];
    out.y0 = [];
    out.rms_x = [];
    out.rms_y = [];
    out.rms_r = [];
    out.final_x = [];
    out.final_y = [];
    out.final_r = [];

    figure('Name',['Error X vs Time - ' label]); hold on; grid on;
    xlabel('Time relative to end (ms)');
    ylabel('Longitudinal error (mm)');
    title(['Longitudinal error vs time - ' label]);

    figure('Name',['Error Y vs Time - ' label]); hold on; grid on;
    xlabel('Time relative to end (ms)');
    ylabel('Lateral error (mm)');
    title(['Lateral error vs time - ' label]);

    figure('Name',['Radial Error vs Time - ' label]); hold on; grid on;
    xlabel('Time relative to end (ms)');
    ylabel('Radial error (mm)');
    title(['Radial error vs time - ' label]);

    for s = 1:length(sheetNames)
        sheet = sheetNames{s};

        % Parse sheet name like x100_y-20
        tok = regexp(sheet, 'x([-+]?\d+)_y([-+]?\d+)', 'tokens', 'once');
        if isempty(tok)
            fprintf('Skipping sheet "%s" in %s (name not in x##_y## format)\n', sheet, filename);
            continue;
        end

        x0 = str2double(tok{1});
        y0 = str2double(tok{2});

        data = readmatrix(filename, 'Sheet', sheet);

        if size(data,2) < 9
            fprintf('Skipping sheet "%s" in %s (not enough columns)\n', sheet, filename);
            continue;
        end

        % Leader B:D
        xL = data(:,2);
        yL = data(:,3);
        tL = data(:,4);

        % Follower G:I
        xF = data(:,7);
        yF = data(:,8);
        tF = data(:,9);

        % Remove NaNs
        validL = ~isnan(xL) & ~isnan(yL) & ~isnan(tL);
        validF = ~isnan(xF) & ~isnan(yF) & ~isnan(tF);

        xL = xL(validL); yL = yL(validL); tL = tL(validL);
        xF = xF(validF); yF = yF(validF); tF = tF(validF);

        if numel(xL) < 5 || numel(xF) < 5
            fprintf('Skipping sheet "%s" in %s (too few points)\n', sheet, filename);
            continue;
        end

        % -----------------------------------
        % Align by end of motion
        % -----------------------------------
        dxL = [0; diff(xL)];
        dxF = [0; diff(xF)];

        moveThresh = 0.5;  % mm per sample
        idxL_end = find(abs(dxL) > moveThresh, 1, 'last');
        idxF_end = find(abs(dxF) > moveThresh, 1, 'last');

        if isempty(idxL_end), idxL_end = length(tL); end
        if isempty(idxF_end), idxF_end = length(tF); end

        tL_rel = tL - tL(idxL_end);
        tF_rel = tF - tF(idxF_end);

        t_start = max(tL_rel(1), tF_rel(1));
        t_end   = min(tL_rel(end), tF_rel(end));

        if t_start >= t_end
            fprintf('Skipping sheet "%s" in %s (no overlap after alignment)\n', sheet, filename);
            continue;
        end

        t_common = t_start:50:t_end;

        xL_i = interp1(tL_rel, xL, t_common, 'linear');
        yL_i = interp1(tL_rel, yL, t_common, 'linear');

        xF_i = interp1(tF_rel, xF, t_common, 'linear');
        yF_i = interp1(tF_rel, yF, t_common, 'linear');

        % -----------------------------------
        % Put leader into follower coordinate frame
        % Leader reverses, so flip sign first
        % -----------------------------------
        xL_F = -xL_i + x0;
        yL_F = -yL_i + y0;

        % -----------------------------------
        % Actual separation
        % -----------------------------------
        sep_x = xL_F - xF_i;
        sep_y = yL_F - yF_i;

        % -----------------------------------
        % Error = deviation from commanded/initial separation
        % -----------------------------------
        err_x = sep_x - x0;
        err_y = sep_y - y0;
        err_r = sqrt(err_x.^2 + err_y.^2);

        % RMS error
        rms_x = sqrt(mean(err_x.^2));
        rms_y = sqrt(mean(err_y.^2));
        rms_r = sqrt(mean(err_x.^2 + err_y.^2));

        % Final error
        final_err_x = err_x(end);
        final_err_y = err_y(end);
        final_err_r = sqrt(final_err_x.^2 + final_err_y.^2);

        % Store results
        out.x0(end+1) = x0;
        out.y0(end+1) = y0;
        out.rms_x(end+1) = rms_x;
        out.rms_y(end+1) = rms_y;
        out.rms_r(end+1) = rms_r;
        out.final_x(end+1) = final_err_x;
        out.final_y(end+1) = final_err_y;
        out.final_r(end+1) = final_err_r;

        % Time-domain plots
        figure(findobj('Name',['Error X vs Time - ' label]));
        plot(t_common, err_x, 'DisplayName', sheet);

        figure(findobj('Name',['Error Y vs Time - ' label]));
        plot(t_common, err_y, 'DisplayName', sheet);

        figure(findobj('Name',['Radial Error vs Time - ' label]));
        plot(t_common, err_r, 'DisplayName', sheet);
    end

    figure(findobj('Name',['Error X vs Time - ' label]));
    yline(0,'--k');
    legend show;

    figure(findobj('Name',['Error Y vs Time - ' label]));
    yline(0,'--k');
    legend show;

    figure(findobj('Name',['Radial Error vs Time - ' label]));
    legend show;
end