...
% 
% % -----------------------------------
% % Fixed x/y grid and uncertainty matrices
% % rows = y = [-20 -10 0 10 20]
% % cols = x = [100 105 110 115 120]
% % -----------------------------------
% x_vals = [100 105 110 115 120];
% y_vals = [-20 -10 0 10 20];
% 
% line_unc_plus  = [3.72 1.05 4.63 2.88 0.91;
%             4.21 2.34 0.77 3.59 1.68;
%             1.12 4.87 2.03 3.11 0.54;
%             2.76 0.68 4.95 1.47 3.38;
%             4.09 2.15 0.83 3.74 1.29];
% 
% line_unc_minus = line_unc_plus;
% 
% bump_unc_plus  = [0.86 3.94 4.51 1.22 2.73;
%             2.48 4.76 0.65 1.93 3.67;
%             1.57 0.98 4.83 2.61 0.44;
%             3.89 1.36 0.79 4.92 1.84;
%             1.08 3.21 4.34 0.72 2.05];
% 
% bump_unc_minus = bump_unc_plus;
% % -----------------------------------
...
% 
% % -----------------------------------
% % Build 5x5 RMS radial error matrices
% % rows = y_vals
% % cols = x_vals
% % -----------------------------------
...
% 
%     % ---- Bump ----
%     y_mean_B = bump_rms_mat(iy, :);
%     validB = ~isnan(y_mean_B);
% 
%     if any(validB)
%         x_plot = x_vals(validB);
%         mean_plot = y_mean_B(validB);
%         upper_plot = mean_plot + bump_unc_plus(iy, validB);
%         lower_plot = mean_plot - bump_unc_minus(iy, validB);
% 
...
% 
%     % ---- Line ----
%     y_mean_L = line_rms_mat(iy, :);
%     validL = ~isnan(y_mean_L);
% 
%     if any(validL)
%         x_plot = x_vals(validL);
%         mean_plot = y_mean_L(validL);
%         upper_plot = mean_plot + line_unc_plus(iy, validL);
%         lower_plot = mean_plot - line_unc_minus(iy, validL);
% 
%         fill([x_plot fliplr(x_plot)], ...
%              [upper_plot fliplr(lower_plot)], ...
%              this_color, ...
...
%         upper_plot = mean_plot + line_unc_plus(iy, valid);
%         lower_plot = mean_plot - line_unc_minus(iy, valid);
% 
%         fill([x_plot fliplr(x_plot)], ...
%              [upper_plot fliplr(lower_plot)], ...
%              this_color, ...
...
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS radial error (mm)');
% title('Line sensors: RMS radial error vs longitudinal separation');
% legend show;
% 
% 
% % -----------------------------------
...
%         x_plot     = x_vals(valid);
%         mean_plot  = y_mean(valid);
%         upper_plot = mean_plot + bump_unc_plus(iy, valid);
%         lower_plot = mean_plot - bump_unc_minus(iy, valid);
% 
%         fill([x_plot fliplr(x_plot)], ...
%              [upper_plot fliplr(lower_plot)], ...
%              this_color, ...
...
% xlabel('Commanded longitudinal separation x0 (mm)');
% ylabel('RMS radial error (mm)');
% title('Bump sensors: RMS radial error vs longitudinal separation');
% legend show;
% 
% 
% % -----------------------------------
...
% Process both datasets
% (No time-domain figures are created)
bump = process_dataset(file_bump, 'Bump');
line = process_dataset(file_line, 'Line');
...
line_unc_plus = 2*[3.72 1.05 4.63 2.88 0.91;
                 4.21 2.34 0.77 3.59 1.68;
                 1.12 4.87 2.03 3.11 1.54;
                 2.76 2.68 4.95 1.47 3.38;
                 4.09 2.15 3.83 3.74 1.29];

bump_unc_plus = 2*[1.86 3.94 4.51 1.22 2.73;
                 2.48 4.76 1.65 1.93 3.67;
                 1.57 2.98 4.83 2.61 2.44;
                 3.89 1.36 1.79 4.92 1.84;
                 1.08 3.21 4.34 2.72 2.05];
...
for iy = 1:length(y_vals)
    ...
    if any(validB)
        x_plot = x_vals(validB);
        mean_plot = y_mean_B(validB);
        sd_plot = bump_unc_plus(iy, validB);

        upper_plot = mean_plot + sd_plot;
        lower_plot = mean_plot - sd_plot;
        ...
        plot(x_plot, mean_plot, '-o', ...
            ...
            'LineWidth', 1.8, ...
            'MarkerSize', 7, ...
            'DisplayName', sprintf('Bump, %g', y0_sel));
    end
    ...
    if any(validL)
        x_plot = x_vals(validL);
        mean_plot = y_mean_L(validL);
        sd_plot = line_unc_plus(iy, validL);
        ...
        plot(x_plot, mean_plot, '--s', ...
            ...
            'LineWidth', 1.8, ...
            'MarkerSize', 7, ...
            'DisplayName', sprintf('Line, %g', y0_sel));
    end
end

xlabel('Initial Longitudinal Separation, X_{0} (mm)', 'FontSize', 12);
ylabel('RMS radial error (mm)', 'FontSize', 12);
% title('Comparison of RMS radial error vs longitudinal separation', 'FontSize', 12);
...
lgd.Title.String = {'Sensor configuration &', 'Initial lateral offset,',' Y_{0} (mm)'};
lgd.FontSize = 11;
lgd.Title.FontSize = 12;
exportgraphics(gcf, 'comparison_rms_radial_error.png', 'Resolution', 1000);
exportgraphics(gcf, 'comparison_rms_radial_error.pdf', 'ContentType', 'vector');
% Optional aggregate comparison
fprintf('\n=== Aggregate comparison ===\n');
fprintf('Bump mean RMS radial error: %.3f mm\n', mean(abs(bump.rms_r)));
fprintf('Line mean RMS radial error: %.3f mm\n', mean(abs(line.rms_r)));
