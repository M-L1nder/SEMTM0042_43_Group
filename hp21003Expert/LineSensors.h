...
class LineSensors_c {
  ...
  public:
   ...
   void calibrateLineSensors( unsigned long duration_ms){
    // initialising min and max
   for (int n = 0; n < NUM_SENSORS; n++) {
       maximum[n] = 0; // max set low
       minimum[n] = 1023; // min set high
   }
   ...
   while (millis() - start < duration_ms){
    ...
    bool isOnBlack(int sensor_num, float threshold = 0.78) { 
      calcCalibratedADC();
