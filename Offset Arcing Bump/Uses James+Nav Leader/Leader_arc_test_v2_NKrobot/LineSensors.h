// Labsheet 2 approach 1 followed, not implemented the supplementary labsheet instructions 
...
// From testing a threshold of 0.7 works well for 'seeing' the black line 
#define sensorthreshold 0.7
...
class LineSensors_c {
  ...
  public:
    ...
    void readSensorsADC() {
      ...
      // initialiseForADC(); // this can be done once in setup so can be commented out here
    ...
    // Two sensor functions - one for any sensor, one for specific - allows turning logic to be tailored to which sensor 
    // Check if a specific sensor is on black
    bool SetOnLine(int which_sensor, float threshold = sensorthreshold) {
      if (which_sensor < 0 || which_sensor >= NUM_SENSORS){ // If sensor number < 0 or sensor number > Number sensors then return false as a safety measure for incorrect inputs 
        return false; // safety for if the 
      }
      return calibrated[which_sensor] >= threshold; // returns true if sensor x is over black
    }

    // Check if any sensor sees black
    bool AnyOnLine(float threshold = sensorthreshold) {
