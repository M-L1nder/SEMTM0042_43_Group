...
bool checkTravelReverse() {
  ...
  float desired_heading = atan2(dy, dx) + PI;
  ...
  left_pwm = applyPwmFloor(left_pwm, -rev + head_corr, PWM_FLOOR);
  right_pwm = applyPwmFloor(right_pwm, -rev - head_corr, PWM_FLOOR);
  ...
  if (lastButtonAState == HIGH && buttonAState == LOW) {
    ...
    if (digitalRead(BUTTON_A_PIN) == LOW) {
      ...
      if (logReadyToDump) {
        ...
        Serial.println(F("i,time_ms,x_mm,y_mm,theta_rad"));
